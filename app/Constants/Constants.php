<?php

namespace App\Constants;

/**
 * Mapa de constantes do sistema.
 *
 * @category Services
 * @package  \App\Services
 * @author   Talysson Lima <diegotalysson@gmail.com>
 * @version  GIT $Id$
 */
final class Constants
{
    const INTEGER               = 'integer';
    const STRING                = 'string';

    const SUCCESS               = 'success';
    const MESSAGE               = 'message';

    const ATUALIZACAO           = 'Atualização';
    const CADASTRO              = 'Cadastro';
    const EXCLUSAO              = 'Exclusão';
    const CD_HISTORICO_PRODUTO  = 'cd_historico_produto';
    const CD_PRODUTO            = 'cd_produtO';
    const CREATED_AT            = 'created_at';
    const DATA                  = 'data';
    const DELETED               = 'deleted';
    const DS_OBSERVACAO         = 'ds_observacao';
    const DS_SKU                = 'ds_sku';
    const NR_QUANTIDADE         = 'nr_quantidade';
    const HISTORICO             = 'historico';
    const IC_REMOVIDO           = 'ic_removido';
    const IC_STATUS             = 'ic_status';
    const ID                    = 'id';
    const OBSERVACAO            = 'observacao';
    const QUANTIDADE_PRODUTOS   = 'quantidade_produtos';
    const PRODUTO               = 'produto';
    const REQUIRED_MAX10_MIN4   = 'required|max:10|min:4';
    const REQUIRED_MAX150_MIN5  = 'required|max:150|min:5';
    const SKU                   = 'sku';
    const STATUS                = 'status';
    const TB_HISTORICO_PRODUTO  = 'tb_historico_produto';
    const TB_PRODUTOS           = 'tb_produtos';
    const TS_ATUALIZADO         = 'ts_atualizado';
    const TS_CRIADO             = 'ts_criado';
    const UPDATED_AT            = 'updated_at';
}
